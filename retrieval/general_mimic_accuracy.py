...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to save the consolidated results CSV file.",
    )
    parser.add_argument(
        "-s",
        "--split-type",
    ...
    if args.resampling:
        ...
        # Save consolidated results with confidence intervals
        save_results(
            mean_ci_results_df,
        ...
        # Save all resampling results for detailed analysis
        save_results(
            all_results_df,
            args.output_results_path.replace(
        ...
        # Combine all results into one consolidated DataFrame
        all_results = []

        # Add accuracy@n results
        for label, metrics in results.items():
            for metric, value in metrics.items():
                all_results.append({"label": label, "metric": metric, "value": value})

        # Add top-k results
        for label, metrics in top_k_results.items():
            for metric, value in metrics.items():
                all_results.append({"label": label, "metric": metric, "value": value})

        # Add DCG results
        for label, metrics in dcg_results.items():
            for metric, value in metrics.items():
                all_results.append({"label": label, "metric": metric, "value": value})
        ...
        save_results(consolidated_df, args.output_results_path)

        print(f"All metrics saved in consolidated format for {args.label_type} labels")
