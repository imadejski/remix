...
# Configuration
BASE_DIR="/opt/gpudata/imadejski/search-model/remix/data/20250804_model_evaluation_mlm_correction"
...
# Output under a separate subdirectory to mark convirt queries
OUT_SUFFIX="convirt_queries"
OUT_BASE_DIR="${BASE_DIR}/${OUT_SUFFIX}"
...
run_embedding_script() {
	...
	# Always reuse existing embeddings for convirt queries
...
run_cosine_script() {
	...
	local output_file="${output_dir}/${model_name}_cosine_similarity_${SPLIT_TYPE}_convirt.csv"
...
run_accuracy_script() {
	...
	local output_results="${model_dir}/${model_name}_accuracy_results_${SPLIT_TYPE}_resampling_${LABEL_TYPE}_convirt.csv"
	local output_all_results="${model_dir}/${model_name}_accuracy_results_${SPLIT_TYPE}_all_resampling_${LABEL_TYPE}_convirt.csv"
	...
	python "${EVAL_SCRIPTS_DIR}/general_mimic_accuracy.py" \
		...
		--output-results-path "${model_dir}/${model_name}_accuracy_results_${SPLIT_TYPE}_convirt.csv" \
...
run_accuracy_no_resample_script() {
	...
	local output_results="${model_dir}/${model_name}_accuracy_results_${SPLIT_TYPE}_convirt_no_resample.csv"
...
process_model() {
	...
	local model_dir_convirt="${OUT_BASE_DIR}/${model_name}"  # convirt outputs go here
	mkdir -p "$model_dir"
	mkdir -p "$model_dir_convirt"
	log "Created/verified directories: $model_dir (embeddings), $model_dir_convirt (convirt outputs)"
	...
	# Step 1: Reuse embeddings (do not rerun)
	local embedding_file
	embedding_file=$(run_embedding_script "$model_name" "$model_checkpoint" "$model_dir")
	if [ $? -ne 0 ]; then
		log "✗ Failed at embedding step for $model_name (expected to reuse existing embeddings)"
		return 1
	fi

	# Step 2: Calculate cosine similarities (convirt queries)
	local cosine_file
	cosine_file=$(run_cosine_script "$model_name" "$model_checkpoint" "$embedding_file" "$model_dir_convirt")
	if [ $? -ne 0 ]; then
	...
	if ! run_accuracy_script "$model_name" "$cosine_file" "$model_dir_convirt"; then
		log "✗ Failed at accuracy resampling step for $model_name"
		return 1
	fi

	# Step 4: Calculate accuracy metrics (without resampling)
	if ! run_accuracy_no_resample_script "$model_name" "$cosine_file" "$model_dir_convirt"; then
...
# Main execution
main() {
	log "Starting BioViL model evaluation process on MIMIC data with impression labels (convirt queries)..."
	log "Embeddings base directory (reused): $BASE_DIR"
	log "Output base directory (convirt): $OUT_BASE_DIR"
	...
	log "Results are stored in individual model folders under: $OUT_BASE_DIR"
}
...
show_help() {
	...
	echo "  --label-type TYPE     Set the label type (default: convirt)"
