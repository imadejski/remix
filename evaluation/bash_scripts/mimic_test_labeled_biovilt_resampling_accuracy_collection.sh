...
for model in "${model_configs[@]}"; do
    ...
    if [ ! -f "$output_path/cosine_similarity.csv" ]; then
        # Run general_mimic_cosine_similarity.py with labeled option
    ...
    if [ ! -f "$output_path/model_top_n_accuracy_results_labeled.csv" ]; then
        # Run general_mimic_accuracy_resampling.py with labeled option
        python data_collection/general_mimic_accuracy_resampling.py \
            --cosine-path "$output_path/cosine_similarity.csv" \
            --output-results-path "$output_path/model_top_n_accuracy_results_labeled.csv" \
            --output-top-k-results-path "$output_path/model_top_k_accuracy_results_labeled.csv" \
