...
from utils import mimic_cxr_get_single_frontal_image_per_study
...
base_model_path = "microsoft/BiomedVLP-BioViL-T"
data_path = "/opt/gpudata/imadejski/mimic-cxr"

output_path = (
    "/opt/gpudata/imadejski/image_text_global_local_alignment/frontal_impression_only"
)
...
df = pd.read_csv(
    os.path.join(
        data_path,
        "mimic_cxr_train_sentence_chunk_sectioned_frontal_impression_only.csv",
    )
)
image_paths = mimic_cxr_get_single_frontal_image_per_study(
    df["study_id"].to_list(), data_path
)
...
for i, avg_patches_after_proj, loss_combo in combinations:
    print(
    ...
    run_output_path = f"{output_path}/run_{i}_{loss_combo}"
    ...
    model_save_path = f"{output_path}/model_run_{i}_{loss_combo}"
